# 5-Step Architecture Spec (Revised v3.4)
...
또한, 각 도메인 모듈은 철저히 격리된 자아를 가집니다. 외부 모듈과의 협업은 오직 서비스(Functor)가 제공하는 명시적 인터페이스 계약을 통해서만 이루어지며, 모듈 내부의 세부 구현과 흐름(Flow)은 외부로 노출되지 않고 은닉됩니다.
 ()
...
- 트랜잭션 원칙: Service는 하나의 유스케이스가 온전히 성립되거나 실패하도록 제어하는 단위이므로, 단일 로컬 트랜잭션의 경계(Boundary)를 확정하는 책임을 가집니다.
...
- Implementation Purity & Double Porting 원칙
    ...
    - Activity 구현체: 어떤 순서로 기술적 도구를 조합할지 정의합니다.(Flow Orchestration)
    - Infra Port: 실제 기술적 행위 자체(외부 시스템/DB 연동 명세)를 정의합니다.
- Infrastructure Selection & Layered Adapter Design 원칙
    ...
    - 물리적 분리: 모든 명세 인터페이스(Port)는 비즈니스 중심 패키지(`activity`, `activity.out`)에 위치하며, 이를 구현하는 모든 실체(Actions, Adapter)는 인프라 및 구현 패키지(`infra.*`)에 두어 컴파일 시점의 의존성을 격리합니다.
